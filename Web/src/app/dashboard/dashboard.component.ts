...
export class DashboardComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.dataService.getAccess().then(() =>
      instance.dataService.getGroups().then(() => {
        instance.pageSizeOpen = Number(
          instance.config.getConfig("pageSizeAssigned")
        );
        instance.pageSizeAvailable = Number(
          instance.config.getConfig("pageSizeAvailable")
        );
        instance.pageSizeSubmitted = Number(
          instance.config.getConfig("pageSizeSubmitted")
        );
        instance.pageSizeDismissed = Number(
          instance.config.getConfig("pageSizeDismissed")
        );
  ...
  openDialog(event: any) {
    ...
    this.dataService
      .getCaseInformation(this.caseId)
  ...
  updateCaseStatus(caseId: string, oldStatus: string, newStatus: string): void {
    ...
    if (
      newStatus == "Assigned" &&
      (oldStatus == "Available" || oldStatus == "Dismissed")
    )
        shouldUpdate = confirm(`I acknowledge that this case has been selected for case review at a Local Overdose Fatality Review Team meeting. I certify that my request for prescription monitoring data (PDMP Data) is for the purpose of furthering an existing, bona fide, individual case review. I certify that the Local Overdose Fatality Review Team reviewing this case has met the requirements to be eligible to request PDMP Data. I understand and agree to comply with the statutory and regulatory requirements for Local Overdose Fatality Review Team access to and use of prescription monitoring data enumerated in Health General Article, Section 21-2A, Annotated Code of Maryland, and Code of Maryland Regulations (COMAR) 10.47.07.\n\n
        Health General Article § 21-2A-06, Annotated Code of Maryland, states that PDMP data are confidential and privileged, are not subject to discovery, subpoena or other means of legal compulsion in civil litigation, are not public records, and except as explicitly allowed within statute may not be disclosed to any person. PDMP Data is subject to additional standards beyond other protected health information (PHI), and Local Overdose Fatality Review Teams should take appropriate measures to ensure its security and prevent any re-disclosure. Re-disclosure of PDMP Data by any member of a Local Overdose Fatality Review Team is not allowed outside the membership of the Team for the purposes of case review conducted under the Team’s authority. All Team members and other individuals granted access to confidential or privileged information will be required to sign the Local Overdose Fatality Review Team’s Confidentiality Agreement, which will be kept on file by the Team coordinator and program staff at MDH. No review of confidential or privileged information shall occur without all attendees having signed the Confidentiality Agreement. 
        `)
      if (shouldUpdate) {
        this.dataService.updateCaseStatus(caseId, newStatus).then(res => {
  ...
  paginationOpenCasesData(event): void {
    ...
    this.dataService
      .getOpenCases(event, this.pageSizeOpen, this.flagFilter)
      .then(d => {
  ...
  paginationAvailableCasesData(event): void {
    ...
    this.dataService
      .getAvailableCases(event, this.pageSizeAvailable, this.flagFilter)
      .then(d => {
  ...
  paginationDismissedData(event): void {
    ...
    this.dataService
      .getDismissedCases(event, this.pageSizeDismissed, this.flagFilter)
      .then(d => {
  ...
  paginationSubmittedData(event): void {
    ...
    this.dataService
      .getSubmittedCases(event, this.pageSizeSubmitted, this.flagFilter)
      .then(d => {
